       ...
       # select template, detection
        ...
        while status:
            ...
            if cords_of_template_abs[2]*cords_of_template_abs[3]*cords_of_detection_abs[2]*cords_of_detection_abs[3] != 0: 
                status = False
            counter_status+=1
            if counter_status > len(sub_class_img_name) : 
                counter_status = 0        
                index_of_subclass = random.choice(range(self.__len__())) # change video if none of the frames contain the target
#             else:
#                 print('Warning : Encounter object missing, reinitializing ...')
    ...
    def _pad_crop_resize(self):
        ...
        if self.check:
            ...
            draw = ImageDraw.Draw(im)
            draw.line([(x1, y1), (x2, y1), (x2, y2), (x1, y2), (x1, y1)], width=1, fill='red')
            save_path = s+'{:04d}.jpg'.format(self.count) #osp.join(s, )
            im.save(save_path)
